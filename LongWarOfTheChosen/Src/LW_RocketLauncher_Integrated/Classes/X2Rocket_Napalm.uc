class X2Rocket_Napalm extends X2Item config(Rockets);
...
static function X2DataTemplate Create_Rocket_Main()
{
	...
	Template.AddAbilityIconOverride('IRI_FireRocket', "img:///IRI_RocketLaunchers.Fire_Napalm");
	Template.AddAbilityIconOverride('IRI_FireRocket_Spark', "img:///IRI_RocketLaunchers.Fire_Napalm");
	Template.AddAbilityIconOverride('IRI_GiveRocket', "img:///IRI_RocketLaunchers.Give_Napalm");
	Template.AddAbilityIconOverride('IRI_ArmRocket', "img:///IRI_RocketLaunchers.Arm_Napalm");
	...
	BurnEffect.EffectTickedFn = BurningTicked;
...
function bool BurningTicked(X2Effect_Persistent PersistentEffect, const out EffectAppliedData ApplyEffectParameters, XComGameState_Effect kNewEffectState, XComGameState NewGameState, bool FirstApplication)
{
	...
	foreach `XCOMHISTORY.IterateByClassType(class'XComGameState_Unit', PlayerUnit)
	{
		...
		if (PlayerUnit.TileDistanceBetween(TargetUnit) > 1)
			continue;
		...
		{
			if (NewGameState.GetContext().PostBuildVisualizationFn.Find(class'X2StatusEffects'.static.BurningSpreadVisualization) == INDEX_NONE)
				NewGameState.GetContext().PostBuildVisualizationFn.AddItem(class'X2StatusEffects'.static.BurningSpreadVisualization);
		}
	}
...
static function X2Effect_Burning CreateBurningSpreadEffect()
{
	local X2Effect_Burning BurningSpreadEffect;
	//local X2Condition_UnitProperty UnitPropertyCondition;
	local X2Condition_Visibility TargetVisibilityCondition;

	// TODO: DISCUSS WITH DESIGN ON WHAT THE SPREAD BURNING DAMAGE SHOULD BE
	BurningSpreadEffect = class'X2StatusEffects'.static.CreateBurningStatusEffect(default.BURN_DAMAGE, default.BURN_DAMAGE_SPREAD);

	BurningSpreadEffect.ApplyChance = default.BURN_DAMAGE_SPREAD_CHANCE;
...
defaultproperties
{
	TemplateName = "IRI_X2Rocket_Napalm"
}