class X2Rocket_WhitePh extends X2Item config(Rockets);
...
static function X2DataTemplate Create_Rocket_Main()
{
	...
	Template.AddAbilityIconOverride('IRI_FireRocket', "img:///IRI_RocketLaunchers.Fire_Phosphorous");
	Template.AddAbilityIconOverride('IRI_FireRocket_Spark', "img:///IRI_RocketLaunchers.Fire_Phosphorous");
	Template.AddAbilityIconOverride('IRI_GiveRocket', "img:///IRI_RocketLaunchers.Give_Phosphorous");
	Template.AddAbilityIconOverride('IRI_ArmRocket', "img:///IRI_RocketLaunchers.Arm_Phosphorous");
...
static function X2Effect CreatePoisonedSpreadEffect()
{
	...
	PoisonSpreadEffect.ApplyChance = class'X2StatusEffects'.default.POISONED_INFECT_PERCENTAGE;
...
function bool PoisonTicked(X2Effect_Persistent PersistentEffect, const out EffectAppliedData ApplyEffectParameters, XComGameState_Effect kNewEffectState, XComGameState NewGameState, bool FirstApplication)
{
	...
	foreach `XCOMHISTORY.IterateByClassType(class'XComGameState_Unit', PlayerUnit)
	{
		...
		if (PlayerUnit.TileDistanceBetween(TargetUnit) > class'X2StatusEffects'.default.POISONED_INFECT_DISTANCE)
			continue;
		...
		{
			if (NewGameState.GetContext().PostBuildVisualizationFn.Find(class'X2StatusEffects'.static.PoisonSpreadVisualization) == INDEX_NONE)
				NewGameState.GetContext().PostBuildVisualizationFn.AddItem(class'X2StatusEffects'.static.PoisonSpreadVisualization);
		}
	}
...
defaultproperties
{
	TemplateName = "IRI_X2Rocket_WhitePh"
}