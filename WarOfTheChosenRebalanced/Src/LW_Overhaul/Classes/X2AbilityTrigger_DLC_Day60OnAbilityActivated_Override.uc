...
function bool AbilityCanTriggerAlienRulerAction(XComGameState_Ability EventAbility, XComGameState GameState, int RulerObjID )
{
	...
	local bool bPathVisibleToRuler, bRulerIsTarget;
	local eAbilityHostility Hostility;

	Hostility = EventAbility.GetMyTemplate().Hostility;
	...
	if(Hostility != eHostility_Offensive && Hostility != eHostility_Defensive && Hostility != eHostility_Neutral)
	{
		// Don't react to movement
		return false;
	}
	...
	//`LOG("Target ID:" @ ActivatedAbilityStateContext.InputContext.PrimaryTarget.ObjectID @ "Ruler ID:" @ RulerObjID @ "Action Hostility:" @ Hostility);

	// If the ruler is the target 
	if (ActivatedAbilityStateContext.InputContext.PrimaryTarget.ObjectID == RulerObjID && Hostility == eHostility_Offensive)
	{
		bRulerIsTarget = true;
	}
	else
	{
		bRulerIsTarget = false;
	}

	// allow non XCOM units to be considered if the Ruler is the direct targed of an offensive ability
	if(Instigator == None || (Instigator.GetTeam() != eTeam_XCom && !bRulerIsTarget))
		return false;
	...
	// only check visibility if the ruler isn't being directly shot at
	if (!bRulerIsTarget)
		{
