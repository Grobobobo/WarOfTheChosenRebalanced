...
static function X2DataTemplate CreateDesperateReserves_LW(){		
	...
	Template.Category = "ResistanceCard";
	Template.ModifyTacticalStartStateFn = RunCheckForMecUnitIfFewerThanSix;

	return Template; 
}

static function RunCheckForMecUnitIfFewerThanSix(XComGameState StartState)
{
	...
	else if (NumSoldiersControlledByPlayer(StartState) < 6)
	{
		GrantResistanceMecAtCombatStart(StartState, BattleData);
	}
