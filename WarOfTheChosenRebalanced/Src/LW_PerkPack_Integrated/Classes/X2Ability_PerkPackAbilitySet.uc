...
class X2Ability_PerkPackAbilitySet extends XMBAbility config (LW_SoldierSkills);
...
var config float FORTIFIED_WOUND_REDUCTION;
var config int FORTIFIED_ABLATIVE;
var config int FORTIFIED_MOBILITY;
...
var config int BULLET_WIZARD_AMMO_COST;
var config float BULLET_WIZARD_DAMAGE_PENALTY;
...
var config float SHADOWSTRIKE_DAMAGE_MOD;

var config int HYPERREACTIVE_PUPILS_AIM_BONUS;
var config int HYPERREACTIVE_PUPILS_CRIT_BONUS;
...
var localized string LocPinnedDebuff;
var localized string LocPinnedDebuffHelpText;
...
static function array<X2DataTemplate> CreateTemplates()
{
	...
	Templates.AddItem(AddFirstInLineAbility());
	...
	Templates.AddItem(AddBulletWizardSuppression());
...
static function X2AbilityTemplate AddMerciless()
{
	...
	Template.eAbilityIconBehaviorHUD = EAbilityIconBehavior_NeverShow;
	Template.Hostility = eHostility_Neutral;
	Template.IconImage = "img:///UILibrary_WOTC_APA_Class_Pack_LW.perk_ShockAndMaul";
	Template.AbilityToHitCalc = default.DeadEye;
	Template.AbilityTargetStyle = default.SelfTarget;
	Template.AbilityTriggers.AddItem(default.UnitPostBeginPlayTrigger);
...
static function X2AbilityTemplate AddFortifiedAbility()
{
	...
	// GreaterPaddingEffect.Padding_HealHP = default.FORTIFIED_WOUND_REDUCTION;	
	// Template.AddTargetEffect(GreaterPaddingEffect);

	// MobBonus = new class'X2Effect_PersistentStatChange';
...
static function X2AbilityTemplate AddFirstInLineAbility()
{
	local X2AbilityTemplate						Template;
	local X2Effect_WilltoSurvive				ArmorBonus;
	`CREATE_X2ABILITY_TEMPLATE (Template, 'FirstInLine');
	Template.IconImage = "img:///UILibrary_LW_PerkPack.LW_AbilityWilltoSurvive";
	Template.AbilitySourceName = 'eAbilitySource_Perk';
	Template.eAbilityIconBehaviorHUD = EAbilityIconBehavior_NeverShow;
	Template.Hostility = eHostility_Neutral;
...
static function X2AbilityTemplate AddHyperReactivePupilsAbility()
{
	...
	HyperReactivePupilsEffect.Aim_Bonus = default.HYPERREACTIVE_PUPILS_AIM_BONUS;
	HyperReactivePupilsEffect.Crit_Bonus = default.HYPERREACTIVE_PUPILS_CRIT_BONUS;
...
static function X2AbilityTemplate AddHairTriggerAbility()
{
	local X2AbilityTemplate                 Template;	
	local X2Effect_HyperReactivePupils		HyperReactivePupilsEffect;
	
	`CREATE_X2ABILITY_TEMPLATE(Template, 'HyperReactivePupils_HT');	
	Template.IconImage = "img:///UILibrary_LW_PerkPack.LW_AbilityHyperreactivePupils";
	Template.AbilitySourceName = 'eAbilitySource_Perk';
	Template.eAbilityIconBehaviorHUD = EAbilityIconBehavior_NeverShow;
	Template.bDisplayInUITacticalText = false;
	...
	Template.AbilityTriggers.AddItem(default.UnitPostBeginPlayTrigger);		
	HyperReactivePupilsEffect = new class'X2Effect_HyperReactivePupils';
	HyperReactivePupilsEffect.Aim_Bonus = default.HAIR_TRIGGER_AIM_BONUS;
	HyperReactivePupilsEffect.Crit_Bonus = default.HAIR_TRIGGER_CRIT_BONUS;
	HyperReactivePupilsEffect.BuildPersistentEffect(1, true, false,, eGameRule_TacticalGameStart);
	HyperReactivePupilsEffect.SetDisplayInfo(ePerkBuff_Passive, Template.LocFriendlyName, Template.GetMyLongDescription(), Template.IconImage, true,,Template.AbilitySourceName);
	Template.AddTargetEffect(HyperReactivePupilsEffect);
	Template.bCrossClassEligible = false;
...
static function X2AbilityTemplate AddBulletWizardAbility()
{
	...
	AmmoCost = new class'X2AbilityCost_Ammo';	
	AmmoCost.iAmmo = default.BULLET_WIZARD_AMMO_COST;
	Template.AbilityCosts.AddItem(AmmoCost);
	...
	PostAbilitySuppression = new class'X2Effect_TriggerEvent';
	PostAbilitySuppression.TriggerEventName = 'ActivateAreaSuppression';
	PostAbilitySuppression.bApplyOnMiss = true;
	PostAbilitySuppression.bApplyOnHit = true;
	PostAbilitySuppression.ApplyChance = 100;
	Template.AddTargetEffect(PostAbilitySuppression);

	Template.AdditionalAbilities.AddItem('BulletWizardSuppression');

	Template.bOverrideAim = true;

	Template.AdditionalAbilities.AddItem('BulletWizardDamage');

	Template.TargetingMethod = class'X2TargetingMethod_AreaSuppression';


	Template.CinescriptCameraType = "Grenadier_SaturationFire";
	Template.ActionFireClass = class'X2Action_Fire_BulletWizard';
...
static function X2AbilityTemplate AddBulletWizardSuppression()
{
	local X2AbilityTemplate								Template;
	local X2AbilityMultiTarget_Radius					RadiusMultiTarget;
	...
	`CREATE_X2ABILITY_TEMPLATE(Template, 'BulletWizardSuppression');
	Template.IconImage = "img:///UILibrary_WOTC_APA_Class_Pack_LW.perk_WitheringBarrage";
	Template.AbilitySourceName = 'eAbilitySource_Perk';
	Template.eAbilityIconBehaviorHUD = eAbilityIconBehavior_NeverShow;
	Template.ShotHUDPriority = class'UIUtilities_Tactical'.const.CLASS_LIEUTENANT_PRIORITY;
	...
	AbilityTrigger.ListenerData.EventID = 'ActivateAreaSuppression';
	...
	PinnedEffect.BuildPersistentEffect(2, false, false, false, eGameRule_PlayerTurnBegin);
	PinnedEffect.DuplicateResponse=eDupe_Refresh;
	PinnedEffect.bRemoveWhenSourceDamaged = false;
	PinnedEffect.SetDisplayInfo(ePerkBuff_Penalty, default.LocPinnedDebuff, default.LocPinnedDebuffHelpText, Template.IconImage, true);
	PinnedEffect.TargetConditions.AddItem(default.LivingHostileUnitOnlyProperty);
	PinnedEffect.TargetConditions.AddItem(new class'X2Condition_CanTakeCover');

	Template.AddTargetEffect(PinnedEffect);
	Template.AddMultiTargetEffect(PinnedEffect);

	//Make it castable on dead peaople, so things won't break in case the primary target dies on sat fire
	//Template.AbilityTargetConditions.AddItem(default.LivingHostileUnitOnlyProperty);

	//SometimesDeadProperty = new class'X2Condition_UnitProperty';
	//SometimesDeadProperty = 
	Template.AbilityTargetConditions.AddItem(default.GameplayVisibilityCondition);
	...
	AbilityCondition = new class 'X2Condition_AbilityProperty';
	AbilityCondition.OwnerHasSoldierAbilities.AddItem('SteadFast');
	...
	Template.BuildVisualizationFn = BwizSuppression_BuildVisualization_LW;
...
static function X2AbilityTemplate BulletWizardDamage()
{
    local X2AbilityTemplate						Template;
	local X2Effect_AbilityDamageMult			DamagePenalty;

    `CREATE_X2ABILITY_TEMPLATE (Template, 'BulletWizardDamage');
    Template.IconImage = "img:///UILibrary_PerkIcons.UIPerk_momentum";
    Template.AbilitySourceName = 'eAbilitySource_Perk';
	...
	DamagePenalty.DamageMod = default.BULLET_WIZARD_DAMAGE_PENALTY;
	DamagePenalty.ActiveAbility = 'BulletWizard';
...
static function X2AbilityTemplate AddSuppressionAbility_LW()
{
	...
	PinnedEffect = new class'X2Effect_Pinned';
	PinnedEffect.BuildPersistentEffect(2, false, false, false, eGameRule_PlayerTurnBegin);
	PinnedEffect.bRemoveWhenTargetDies = false;
	PinnedEffect.bRemoveWhenSourceDamaged = false;
	PinnedEffect.DuplicateResponse=eDupe_Refresh;
	PinnedEffect.SetDisplayInfo(ePerkBuff_Penalty, default.LocPinnedDebuff, default.LocPinnedDebuffHelpText, Template.IconImage, true);
	PinnedEffect.TargetConditions.AddItem(new class'X2Condition_CanTakeCover');
...
simulated function BwizSuppression_BuildVisualization_LW(XComGameState VisualizeGameState)
{
	local XComGameStateHistory			History;
	local XComGameStateContext_Ability  Context;
	local StateObjectReference          InteractingUnitRef;
	...
	local bool 							AllTargetsDead;
	History = `XCOMHISTORY;

	//for BWIZ suppression, you'll need to check if all of the targets are dead (may have beem killed off by the sweep) - then you should not start the visualization
	//Sometimes the convoluted shit I have to pull off to make stuff work is infuriating and then it ends up not working anyway reeeeee
	AllTargetsDead = true;
	...
	{
		TargetUnit = XComGameState_Unit(VisualizeGameState.GetGameStateForObjectID(Context.InputContext.MultiTargets[i].ObjectID));
		if(TargetUnit.IsAlive())
		{
			AllTargetsDead = false;
			break;
...
{
	local X2AbilityTemplate						Template;
	local X2Effect_ShadowStrike ShadowStrikeEffect;
	...
	ShadowStrikeEffect = new class'X2Effect_Shadowstrike';
	ShadowStrikeEffect.BuildPersistentEffect(1, true, false);
	ShadowStrikeEffect.SetDisplayInfo (ePerkBuff_Passive, Template.LocFriendlyName, Template.GetMyHelpText(), Template.IconImage,,, Template.AbilitySourceName);
	ShadowStrikeEffect.SS_AimBonus = class'X2Ability_RangerAbilitySet'.default.SHADOWSTRIKE_AIM;
	ShadowStrikeEffect.SS_DamageBonus = default.SHADOWSTRIKE_DAMAGE_MOD;
	Template.AddTargetEffect(ShadowStrikeEffect);
