...
var config int TARGET_FOCUS_DODGE_REDUCTION;
var config int TARGET_FOCUS_DEFENSE_REDUCTION;
...
var config float ABSORPTION_FIELDS_DAMAGE_REDUCTION;
...
{
	local X2Effect_TargetFocus ShootingEffect;
	local X2AbilityTemplate Template;

	// Create an armor piercing bonus
	ShootingEffect = new class'X2Effect_TargetFocus';
	ShootingEffect.DodgeReductionBonus = default.TARGET_FOCUS_DODGE_REDUCTION;
	ShootingEffect.DefenseReductionBonus = default.TARGET_FOCUS_DEFENSE_REDUCTION;
...
static function X2AbilityTemplate Magnum()
{
	...
	return Passive('Magnum_LW', "img:///UILibrary_XPerkIconPack_LW.UIPerk_pistol_sniper", false, ShootingEffect);
}
...
{
	local X2AbilityTemplate					Template;
	local X2Effect_Entwine			DefenseEffect;

	`CREATE_X2ABILITY_TEMPLATE (Template, 'Entwine_LW');
	Template.IconImage = "img:///UILibrary_XPerkIconPack_LW.UIPerk_enemy_defense";
	Template.AbilitySourceName = 'eAbilitySource_Perk';
	Template.Hostility = eHostility_Neutral;
	Template.eAbilityIconBehaviorHUD = eAbilityIconBehavior_NeverShow;
	Template.AbilityToHitCalc = default.DeadEye;
	...
	DefenseEffect = new class'X2Effect_Entwine';
	DefenseEffect.Defense = default.ENTWINE_DEF;
...
static function X2AbilityTemplate Dominant()
{
	...
	// Activated ability that targets user
	Template = Passive('Dominant_LW', "img:///UILibrary_XPerkIconPack_LW.UIPerk_enemy_command_psi", true);
...
static function X2AbilityTemplate Grit()
{
	...
	// Activated ability that targets user
	Template = Passive('Grit_LW', "img:///UILibrary_XPerkIconPack_LW.UIPerk_shield_plus", true);
...
static function X2AbilityTemplate CreateQuickRetreat()
{
	...
	Trigger.ListenerData.EventFn = QuickRetreatListener;
...
static function EventListenerReturn QuickRetreatListener(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{
	local XComGameStateContext_Ability AbilityContext;
	...
	//Check if the source is flanked
	if (AbilityContext != None && AbilityContext.InterruptionStatus != eInterruptionStatus_Interrupt)
	{
		//	were we the killer?
		if (TargetUnit.IsFlanked(SourceUnit.GetReference()))
		{
...
static function X2AbilityTemplate CreateAbsorptionFields()
{
	...
	Effect.DamageReduction = default.ABSORPTION_FIELDS_DAMAGE_REDUCTION;
...
static function X2AbilityTemplate AddSuperiorHolyWarrior()
{
	...
	Template = PurePassive('SuperiorHolyWarrior', "img:///UILibrary_XPerkIconPack_LW.UIPerk_gremlin_psi", true, 'eAbilitySource_Perk');
	Template.bCrossClassEligible = false;
	//Template.AdditionalAbilities.AddItem('DamageControlAbilityActivated');
	return Template;
}
...
static function X2AbilityTemplate ShredderRoundsDamagePenalty()
{
	...
	return Passive('ShredderRoundsPenalty', "img:///UILibrary_XPerkIconPack_LW.UIPerk_melee_adrenaline", true, Effect, false);
}

	static function X2AbilityTemplate PrimaryGrenadeLauncherPassive(name TemplateName)
{	
	return Passive(TemplateName, "img:///UILibrary_XPerkIconPack_LW.UIPerk_melee_adrenaline", true, , false);
}
...
{	
	...
	Template = Passive('GremlinBonusAnim', "img:///UILibrary_XPerkIconPack_LW.UIPerk_melee_adrenaline", true, , false);
...
static function X2AbilityTemplate CreateBerserkerBladestorm()
{
	local X2AbilityTemplate Template;
	Template = PurePassive('BerserkerBladestorm', "img:///UILibrary_PerkIcons.UIPerk_beserker_rage", false, 'eAbilitySource_Perk');
	Template.AdditionalAbilities.AddItem('BerserkerBladestormAttack');
	return Template;
}

static function X2DataTemplate CreateBerserkerBladestormAttack()
{
	local X2AbilityTemplate Template;
	local X2Condition_NotItsOwnTurn NotItsOwnTurnCondition;
	local X2Condition_Unitproperty RangeCondition;
	Template = class'X2Ability_RangerAbilitySet'.static.BladestormAttack('BerserkerBladestormAttack');
