...
//-----------------------------------------------------------
class X2Action_Fire_BulletWizard extends X2Action_Fire;
...
var array<AvailableTarget> MultiTargets;
...
function Init()
{
    ...
    local XcomGameState_Unit PrimaryTargetUnit;
	super.Init();
	...
	if (AbilityState.GetMyTemplate().TargetingMethod == class'X2TargetingMethod_AreaSuppression')
	{
        ...
        PrimaryTargetUnit = XComGameState_Unit(`XCOMHISTORY.GetGameStateForObjectID(AbilityContext.InputContext.PrimaryTarget.ObjectID));

        //to get the furthest distance from source, first check the primary target, then the multi target, and use the max value from those
        TargetLoc = UnitPawn.Location;
        UnitLoc = WorldData.GetPositionFromTileCoordinates(PrimaryTargetUnit.TileLocation);
        EndLocation = UnitLoc;
        MaxDist = VSize(TargetLoc - UnitLoc);

        for( i = 0; i< AbilityContext.InputContext.TargetLocations.Length; i++)
        {

            UnitLoc = AbilityContext.InputContext.TargetLocations[i];
            Dist = VSize(TargetLoc - UnitLoc);
            if(MaxDist < Dist)
            {
                MaxDist = Dist;
                EndLocation = UnitLoc;
        ...
        //For cone length, use the furthest time from the start location
		ConeLength = MaxDist;


		// Widen the cone a little bit so the sweep looks more natural when hitting outer units
		ConeWidth = radiusTemplate.GetTargetRadius(AbilityState) * 2;
        ...
        TargetsLeftToNotify.AddItem(AbilityContext.InputContext.PrimaryTarget);
	}
