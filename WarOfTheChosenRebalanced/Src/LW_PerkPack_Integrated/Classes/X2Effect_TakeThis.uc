...
class X2Effect_TakeThis extends X2Effect_TemporaryItem;
...
{
	local name									UseItemName, AltItemName,OGWeaponTemplateName;
	local XComGameState_Unit					UnitState; 
	local XComGameState_Item					OldItemState, UpdatedItemState, NewItemState, SourceWeapon;
	...
	OGWeaponTemplateName = SourceWeapon.GetMyTemplateName();

	UseItemName = OGWeaponTemplateName;
...
function array<name> GatherAbilitiesForItem(X2EquipmentTemplate EquipmentTemplate)
{
	...
	if(X2WeaponTemplate(EquipmentTemplate) != none)
	{
		if(X2WeaponTemplate(EquipmentTemplate).WeaponCat == 'pistol')
		{
		...
		{
			EquipmentAbilities.AddItem('FanFire');
			EquipmentAbilities.AddItem('Predator_LW');
			EquipmentAbilities.AddItem('LightningHands');

		}
		else if(X2WeaponTemplate(EquipmentTemplate).WeaponCat == 'sawedoffshotgun')
		{
			EquipmentAbilities.AddItem('CloseEncounters');
			EquipmentAbilities.AddItem('HuntersInstinct');
			EquipmentAbilities.AddItem('Infighter');
		}
	}
